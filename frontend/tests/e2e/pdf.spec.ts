...
test("Download CV button has correct href for current lang+theme", async ({ page }) => {
  ...
  await expect(downloadLink).toHaveAttribute("href", "/api/cv/pdf/?lang=en&theme=normal");

  // Switch to DE
  await page.getByRole("button", { name: /Switch language|Sprache wechseln/i }).click();
  await expect(downloadLink).toHaveAttribute("href", "/api/cv/pdf/?lang=de&theme=normal");

  // Switch to dog
  await page.getByRole("button", { name: /Toggle theme|Theme wechseln/i }).click();
  await expect(downloadLink).toHaveAttribute("href", "/api/cv/pdf/?lang=de&theme=dog");
});
