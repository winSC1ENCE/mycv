...
- Server-rendered **PDF export** via WeasyPrint (admin-only, EN/DE) — the full CV in a modern two-column layout, a **Certificates PDF** bundling the scans attached to Experience/Education entries (image attachments embedded inline, PDF attachments merged in via `pypdf`), plus per-application documents (below). Triggered from the admin Dashboard **Exports** card.
